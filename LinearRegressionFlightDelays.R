# Regression analysis "FLIGHT DELAYS"

flights.data <- read.csv("D:/STUDIES/Big Data/R/Projects_for_R/Data Science Project_4/Data Science Project_Regression/FlightDelays.csv")
head(flights.data)
set.seed(1234)
## define hours of departure
flights.data$schedtime = floor(flights.data$schedtime/100)
flights.data$schedtime
table(flights.data$schedtime)
table(flights.data$carrier)
table(flights.data$dest)
table(flights.data$origin)
table(flights.data$weather)
table(flights.data$dayweek)
table(flights.data$daymonth)
table(flights.data$delay)

## needed to recode variables
library(car)
flights.data$delay = recode(flights.data$delay,"'delayed'=1;'ontime'=0")
flights.data$delay = as.numeric(levels(flights.data$delay)[flights.data$delay])
table(flights.data$delay)
class(flights.data$delay)
## Delay: 1=Monday; 2=Tuesday; 3=Wednesday; 4=Thursday;
## 5=Friday; 6=Saturday; 7=Sunday
## 7=Sunday and 1=Monday coded as 1
flights.data$dayweek <- recode(flights.data$dayweek,"c(1,7)=1;else=0")
table(flights.data$dayweek)
## omit unused variables
flights.data=flights.data[,c(-1,-3,-5,-6,-7,-11,-12)]
flights.data[1:3,]
n=length(flights.data$delay)
n
n1=floor(n*(0.6))
n1
n2=n-n1
...
Xdel <- model.matrix(delay~.,data=flights.data)[,-1]
Xdel[1:3,]
xtrain <- Xdel[train,]
xnew <- Xdel[-train,]
ytrain <- flights.data$delay[train]
ynew <- flights.data$delay[-train]
